    ...
    if __name__ == "__main__":
        try:
            ...
            if args.old_logo is None:
                new_logo = Image.open(logo).convert("RGBA")
                if new_logo.size != (308, 350):
                    Invalid = True #raise ValueError("Invalid size for the logo")
            ...
            if Invalid == True:
                root = tk.Tk()
                tk.Label(root, text="Invalid size for the logo, Must be 308x350").grid( row = 0, column = 0, padx=2, pady=2)
                tk.Button(root, text="  OK  ", command=root.destroy).grid( row = 1, column = 0, padx=2, pady=2)
            ...
            root = tk.Tk()
            tk.Label(root, text=e).grid( row = 0, column = 0, padx=2, pady=2)
            tk.Button(root, text="  OK  ", command=root.destroy).grid( row = 1, column = 0, padx=2, pady=2)
            root.bind('<Return>', root.destroy)
            root.mainloop()
...
root = tk.Tk()
root.title("Gen_Patches GUI")


tk.Label(root, text="GUI Logo Patcher for switch").grid(row = 0, column = 1, columnspan = 3, padx = 100, pady =  15)
tk.Label(root, text="New Logo :").grid(row = 1, column = 1, padx = 5, pady = 5)
entry = tk.Entry(root, width = 25)
entry.grid(row = 1, column = 2, padx = 5, pady = 5)

Button = tk.Button(text = "     Enter     ", command = patch)
Button.grid(row = 2, column = 3, padx = 5, pady = 5)


root.bind('<Return>', patch)
